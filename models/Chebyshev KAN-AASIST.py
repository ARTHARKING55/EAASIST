...
class KANLinear(nn.Module):
    def __init__(
        ...
        scale_base=1.0,
        scale_cheb=1.0,
        enable_standalone_scale_cheb=True,
        ...
        self.enable_standalone_scale_cheb = enable_standalone_scale_cheb
        self.grid_eps = grid_eps
        self.grid_range = grid_range

        self.base_weight = nn.Parameter(torch.Tensor(out_features, in_features))
        self.cheb_weight = nn.Parameter(torch.Tensor(out_features, in_features, degree + 1))
        if enable_standalone_scale_cheb:
            self.cheb_scaler = nn.Parameter(torch.Tensor(out_features, in_features))
        else:
            self.register_buffer("cheb_scaler", None) 
        
        self.base_activation = base_activation().to(self.device)
        self.register_buffer("cheb_coeffs", self._precompute_cheb_coeffs(degree))
        self.reset_parameters()

    def reset_parameters(self):
        nn.init.kaiming_uniform_(self.base_weight, a=math.sqrt(5) * self.scale_base * 0.1)
        nn.init.kaiming_uniform_(self.cheb_weight, a=math.sqrt(5) * self.scale_cheb * 0.1)
        if self.enable_standalone_scale_cheb and self.cheb_scaler is not None:
            nn.init.kaiming_uniform_(self.cheb_scaler, a=math.sqrt(5) * self.scale_cheb * 0.1)
        with torch.no_grad():
            noise = (
                (torch.rand(self.out_features, self.in_features, self.degree + 1) - 0.5)
                * self.scale_noise
                / self.in_features
            )
            self.cheb_weight.data.copy_(
                (self.scale_cheb if not self.enable_standalone_scale_cheb else 1.0) * noise
            )

    def _precompute_cheb_coeffs(self, degree):
        coeffs = torch.zeros(degree + 1, degree + 1, device=self.device)
        coeffs[0, 0] = 1.0  # T_0(x) = 1
        if degree >= 1:
            coeffs[1, 1] = 1.0  # T_1(x) = x
        for n in range(2, degree + 1):
            coeffs[n, 0:n-1] = -coeffs[n-2, 0:n-1]  # -T_{n-2}
            coeffs[n, 1:n+1] += 2 * coeffs[n-1, 0:n]  # 2 * x * T_{n-1}
        return coeffs

    def chebyshev_polynomials(self, x):
        x = x.to(self.device)
        range_width = max(self.grid_range[1] - self.grid_range[0], 1e-8)
        x_normalized = (2 * (x - self.grid_range[0]) / range_width) - 1
        x_normalized = torch.clamp(x_normalized, -1, 1)
        
        batch_size, in_features = x_normalized.shape
        x_powers = torch.ones(batch_size, in_features, self.degree + 1, device=self.device)
        for d in range(1, self.degree + 1):
            x_powers[:, :, d] = x_normalized ** d
        cheb_terms = torch.einsum('bid,dn->bin', x_powers, self.cheb_coeffs)
        cheb_terms = torch.clamp(cheb_terms, -1e6, 1e6)
        return cheb_terms  

    @property
    def scaled_cheb_weight(self):
        return self.cheb_weight * (
            self.cheb_scaler.unsqueeze(-1) if self.enable_standalone_scale_cheb and self.cheb_scaler is not None else 1.0
        )

    def forward(self, x: torch.Tensor):
        assert x.size(-1) == self.in_features, f"Expected input feature size {self.in_features}, got {x.size(-1)}"
        x = x.to(self.device)
        ...
        cheb_weight = self.scaled_cheb_weight.view(self.out_features, -1)
        cheb_terms = cheb_terms.view(-1, self.in_features * (self.degree + 1))
        cheb_proj = torch.matmul(cheb_terms, cheb_weight.t())
    ...
    def update_grid(self, x: torch.Tensor, margin=0.01):
        ...
        grid_adaptive = x_sorted[
            torch.linspace(0, batch - 1, self.in_features + 1, 
                         dtype=torch.int64, device=x.device)
        ]
        uniform_step = max((x_sorted[-1] - x_sorted[0] + 2 * margin) / self.in_features, 1e-8)
        grid_uniform = (
            torch.arange(self.in_features + 1, dtype=torch.float32, device=x.device).unsqueeze(1)
            * uniform_step + x_sorted[0] - margin
        )
        grid = self.grid_eps * grid_uniform + (1 - self.grid_eps) * grid_adaptive
        self.base_weight.data.copy_(self.base_weight.data + (grid[:, :-1].T - self.base_weight.data) * self.grid_eps)

    def load_state_dict(self, state_dict, strict=True):
        own_state = self.state_dict()
        for name, param in state_dict.items():
            if name in own_state:
                if name == "cheb_weight" and param.shape[-1] != self.cheb_weight.shape[-1]:
                    old_degree = param.shape[-1] - 1
                    new_degree = self.degree
                    if old_degree < new_degree:
                        pad = torch.zeros(param.shape[0], param.shape[1], new_degree - old_degree, device=param.device)
                        param = torch.cat([param, pad], dim=-1)
                    else:
                        param = param[:, :, :new_degree + 1]
                    own_state[name].copy_(param)
                elif name == "cheb_scaler" and self.cheb_scaler is None:
                    continue  
                else:
                    own_state[name].copy_(param)
        if not strict:
...
class GraphAttentionLayer(nn.Module):
    def __init__(self, in_dim, out_dim, **kwargs):
        super().__init__()
        self.att_proj = KANLinear(in_dim, out_dim) # KANLinear replaces nn.Linear everywhere
