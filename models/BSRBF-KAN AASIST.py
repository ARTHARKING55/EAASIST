...
class KANLinear(nn.Module):
    def __init__(
        ...
        self.spline_weight = nn.Parameter(
            torch.empty(output_dim, input_dim * (grid_size + spline_order))
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        bspline_out = self.b_splines(x).reshape(x.size(0), -1)
        rbf_out = self.rbf(x).reshape(x.size(0), -1)
        combined_basis = bspline_out + rbf_out
    
        nonlinear_out = F.linear(combined_basis, self.spline_weight)
        output = base_out + nonlinear_out
    
        return output.reshape(*original_shape[:-1], self.output_dim) 
